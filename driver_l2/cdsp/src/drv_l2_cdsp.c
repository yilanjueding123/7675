...
#elif (USE_SENSOR_NAME == SENSOR_OV9712)//SENSOR_OV9712
...
#elif  (USE_SENSOR_NAME == SENSOR_SOI_H42)	
	extern sensor_exposure_t *jxh42_get_senInfo(void);
	extern sensor_calibration_t *jxh42_get_calibration(void);
	extern int H42_set_exposure_time(sensor_exposure_t *si);
	extern void H42_set_exposure_gain(void);
	extern void H42_get_exposure_time(sensor_exposure_t *se);
	extern void sensor_SOi_h42_init(INT32U WIDTH, INT32U HEIGHT);
	
	extern int H42_get_night_ev_idx(void);
	extern int H42_get_max_ev_idx(void);
	extern void H42_set_exp_freq(int freq);		
	extern gpCdspWBGain_t *soi_h42_awb_r_b_gain_boundary(void);	
	extern void SOi_h42_sensor_calibration(void);
#elif (USE_SENSOR_NAME == SENSOR_OV9732)
	extern sensor_exposure_t *ov9732_get_senInfo(void);
	extern sensor_calibration_t *ov9732_get_calibration(void);
	extern int ov9732_set_exposure_time(sensor_exposure_t *si);
	extern void ov9732_set_exposure_gain(void);
	extern void ov9732_get_exposure_time(sensor_exposure_t *se);
	extern void sensor_ov9732_init(INT32U format, INT32U width, INT32U height);
	extern void ov9732_sensor_calibration(void);
	extern int ov9732_get_night_ev_idx(void);
	extern int ov9732_get_max_ev_idx(void);
	extern void ov9732_set_exp_freq(int freq);
	extern gpCdspWBGain_t *ov9732_awb_r_b_gain_boundary(void);		
#elif (USE_SENSOR_NAME == SENSOR_GC1014)
	extern sensor_exposure_t *gc1014_get_senInfo(void);
	//extern void gc1014_get_senInfo2(sensor_exposure_t *pSensorInfo) ;
	extern sensor_calibration_t *gc1014_get_calibration(void);
	extern int gc1014_set_exposure_time(sensor_exposure_t *si);
	extern void gc1014_set_exposure_gain(void);
	extern void gc1014_get_exposure_time(sensor_exposure_t *se);
	extern void sensor_gc1014_init(INT32U WIDTH, INT32U HEIGHT);
	
	extern int gc1014_get_night_ev_idx(void);
	extern int gc1014_get_max_ev_idx(void);
	extern void gc1014_set_exp_freq(int freq);
	extern gpCdspWBGain_t *gc1014_awb_r_b_gain_boundary(void);	
...
#define DENOISE_SELECT 		HYBRID_DENOISE_ONLY
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#define USE_INDEPEDENT_TASK_FOR_AE		1

#if USE_INDEPEDENT_TASK_FOR_AE

#define AE_SERVICE_TASK_STACK_SIZE		1024
#define AE_SERVICE_TASK_QUEUE_MAX		16

#define	MSG_DO_SENSOR_AE		0x5a5a
#define	MSG_DO_CDSP_AE			0x3838
...
void gp_isp_iso_set(INT32U iso)
{
	...
	#elif (USE_SENSOR_NAME == SENSOR_SOI_H42)	
		H42_get_exposure_time(&seInfo);	
	#elif (USE_SENSOR_NAME == SENSOR_OV9732)
		ov9732_get_exposure_time(&seInfo);			
	#elif (USE_SENSOR_NAME == SENSOR_GC1014)
		gc1014_get_exposure_time(&seInfo);
	#endif
...
void gp_cdsp_set_exp_freq(INT32U freq_num)
{
	...
	#elif (USE_SENSOR_NAME == SENSOR_SOI_H42)	
		H42_set_exp_freq(exp_freq);		
	#elif (USE_SENSOR_NAME == SENSOR_OV9732)
		ov9732_set_exp_freq(exp_freq);		
	#elif (USE_SENSOR_NAME == SENSOR_GC1014)
		gc1014_set_exp_freq(exp_freq);		
	#endif
...
static void gp_cdsp_handle_eof(void)
{
	...
	if (CdspDev->ae_ret == AE_CHANGE) 
	{
		...
		//{
			...
			gp_cdsp_set_exposure_time();
		//}
	  #endif
	}
#if (ENABLE_SAVE_SENSOR_RAW_DATA == 0)
...
#elif  (USE_SENSOR_NAME == SENSOR_SOI_H42)
	p_seInfo = jxh42_get_senInfo();
	p_seCalibration	= jxh42_get_calibration();
#elif  (USE_SENSOR_NAME == SENSOR_OV9732)
	p_seInfo = ov9732_get_senInfo();
	p_seCalibration = ov9732_get_calibration();		
#elif  (USE_SENSOR_NAME == SENSOR_GC1014)
	p_seInfo = gc1014_get_senInfo();
	p_seCalibration = gc1014_get_calibration();	
#endif
...
#elif  (USE_SENSOR_NAME == SENSOR_SOI_H42)
    p_WB_Gain =soi_h42_awb_r_b_gain_boundary();    
  //  DBG_PRINT("max_rgain= %d, max_bgain= %d,min_rgain= %d, min_bgain= %d\r\n", p_WB_Gain->max_rgain, p_WB_Gain->max_bgain,  p_WB_Gain->min_rgain,  p_WB_Gain->min_bgain);    
    gp_cdsp_awb_set_r_b_gain_boundary(awb, p_WB_Gain->max_rgain, p_WB_Gain->max_bgain,  p_WB_Gain->min_rgain,  p_WB_Gain->min_bgain);

#elif  (USE_SENSOR_NAME == SENSOR_OV9732)
    p_WB_Gain =ov9732_awb_r_b_gain_boundary();    
  //  DBG_PRINT("max_rgain= %d, max_bgain= %d,min_rgain= %d, min_bgain= %d\r\n", p_WB_Gain->max_rgain, p_WB_Gain->max_bgain,  p_WB_Gain->min_rgain,  p_WB_Gain->min_bgain);    
    gp_cdsp_awb_set_r_b_gain_boundary(awb, p_WB_Gain->max_rgain, p_WB_Gain->max_bgain,  p_WB_Gain->min_rgain,  p_WB_Gain->min_bgain);
#elif  (USE_SENSOR_NAME == SENSOR_GC1014)
    p_WB_Gain =gc1014_awb_r_b_gain_boundary();    
  //  DBG_PRINT("max_rgain= %d, max_bgain= %d,min_rgain= %d, min_bgain= %d\r\n", p_WB_Gain->max_rgain, p_WB_Gain->max_bgain,  p_WB_Gain->min_rgain,  p_WB_Gain->min_bgain);    
    gp_cdsp_awb_set_r_b_gain_boundary(awb, p_WB_Gain->max_rgain, p_WB_Gain->max_bgain,  p_WB_Gain->min_rgain,  p_WB_Gain->min_bgain);
	...
	gp_cdsp_ae_set_target_lum_night(CdspDev->ae_workmem, TARGET_Y_NIGHT);
}
		...
		if((CdspDev->ae_awb_flag & CDSP_AEAWB_CTRL_EN) != 0)
		{
			...
			{			
				...
				if(ae_frame >= ae_frame_thr)
				{
					...
					#elif (USE_SENSOR_NAME == SENSOR_SOI_H42)	
						H42_get_exposure_time(&seInfo);
					#elif (USE_SENSOR_NAME == SENSOR_SOI_H62)	
						H62_get_exposure_time(&seInfo);		
					#elif (USE_SENSOR_NAME == SENSOR_OV9732)
						ov9732_get_exposure_time(&seInfo);	
					#elif (USE_SENSOR_NAME == SENSOR_GC1014)
						gc1014_get_exposure_time(&seInfo);
				...
				/*		DBG_PRINT("\r\n\r\n\r\nAE win output:");
					...
					if(seInfo.ae_ev_idx != 0)
					{
						...
						if(gp_cdsp_ae_is_night_mode(ae) == 0 && CdspDev->sat_contr_idx != -1) 
						{
							CdspDev->sat_contr_idx = -1;
							CdspDev->ae_awb_flag |= CDSP_SAT_SWITCH;
						}
						else if(current_ev_idx >= sat_yuv_thr[3] && CdspDev->sat_contr_idx != 3)
						{							
			...
			{		
				...
				if(awb_frame >= 2)
				{
					...
					*/
					...
					//DBG_PRINT("\nret = %d, awb sum cnt = %d\r\n",  ret, p_3a_result->awb_result.sumcnt[0] +  p_3a_result->awb_result.sumcnt[1] +  p_3a_result->awb_result.sumcnt[2]);
...
*****************************************************/
#if (ENABLE_SAVE_SENSOR_RAW_DATA || CDSP_DEBUG)
...
#if ((sensor_format == GC1004_MIPI) ||(sensor_format == GC1014_MIPI) || (sensor_format == SOI_H22_MIPI)||(sensor_format == SOI_H42_MIPI)||(sensor_format == OV9732_MIPI))
...
#elif (USE_SENSOR_NAME == SENSOR_SOI_H42)		
	step = sat_yuv_thr[3] = H42_get_max_ev_idx() - 4;
	sat_yuv_thr[0] = H42_get_night_ev_idx();
#elif (USE_SENSOR_NAME == SENSOR_OV9732)	
	step = sat_yuv_thr[3] = ov9732_get_max_ev_idx() - 4;
	sat_yuv_thr[0] = ov9732_get_night_ev_idx();				
#elif (USE_SENSOR_NAME == SENSOR_GC1014)	
	step = sat_yuv_thr[3] = gc1014_get_max_ev_idx() - 4;
	sat_yuv_thr[0] = gc1014_get_night_ev_idx();		
#endif
	step -= sat_yuv_thr[0];
	step = (step + 1) / 3;	
...
void gp_isp_start(INT32U dummy_addr,INT32U gpSENSOR_WIDTH, INT32U gpSENSOR_HEIGHT)	
{
	...
	R_CDSP_DMA_RAWBUF_SADDR = 0xF8500000;
	R_CDSP_MD_DMA_SADDR = 0xF8500000;
	R_CDSP_FRONT_DUMMY = 0xF8500000; 
	#endif
	CdspDev = &gp_CdspDev;
	...
	hwFront_SetInputFormat(SENSOR_INTERFACE);	//set cdsp input format
	...
	#elif (USE_SENSOR_NAME == SENSOR_SOI_H42)
	SOi_h42_sensor_calibration();
	#elif (USE_SENSOR_NAME == SENSOR_OV9732)
	ov9732_sensor_calibration();
	#elif (USE_SENSOR_NAME == SENSOR_GC1014)
	gc1014_sensor_calibration();
	#else
	DBG_PRINT("Non calibration sensor\r\n");
	#endif
#endif	
	hwCdsp_EnableAF(DISABLE, ENABLE);
	hwCdsp_EnableAE(DISABLE, ENABLE);
	hwCdsp_EnableAWB(DISABLE, ENABLE);
	...
	/*--- wb gain config ---*/
	gp_cdsp_set_wb_gain(ENABLE, DISABLE);
...
#else
	/*--- ISP LSC ---*/
	#if LENS_CMP_CTRL //If Enable LSC must be set 0
	...
	/*--Edge--*/
	/*new denoise config*/
	gp_cdsp_set_new_denoise(NIGHT_EDGE);
	/*set Hybrid Denoise*/
...
//#endif
	...
	hwCdsp_SetExtLine(ENABLE, gpSENSOR_WIDTH, 0x80);
	...
	{
		INT32U hfactor, vfactor;
		hfactor = ((320+1) << 16)/gpSENSOR_WIDTH + 1;
		vfactor = ((240+1) << 16)/gpSENSOR_HEIGHT + 1;
		clamp_en = 1;
	...
	hwCdsp_SetRawDataFormat(2);   //BGGR
	#endif
	hwFront_SetFrameSize(0x138, 0x02, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	//pll 36,BGGR
	
	sensor_SOi_h42_init(gpSENSOR_WIDTH, gpSENSOR_HEIGHT);
	...
	hwFront_SetFrameSize(0x07, 0x02, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	//BGGR
	#endif	
	sensor_gc1014_init(gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	
#else
	DBG_PRINT("\r\n= Non Sensor Init =\r\n");
#endif
...
void gp_mipi_isp_start(INT32U dummy_addr, INT32U gpSENSOR_WIDTH, INT32U gpSENSOR_HEIGHT)	
{
	...
	hwCdsp_SetYuvRange(0x0); 				//unsigned
	hwCdsp_DataSource(MIPI_INPUT);
	hwFront_SetInputFormat(SENSOR_INTERFACE);
...
//#endif
	...
	//Enable raw interpolution
	hwCdsp_SetExtLine(ENABLE, gpSENSOR_WIDTH, 0x80);
	hwCdsp_IntplThrSet(24, 240); // same as GP12B
	//hwCdsp_IntplThrSet(128, 240); // same as GP12B
	hwCdsp_EnableIntplMir(0x0F, 1, 0);	//intplmirvsel,	//suggest set 1
	read_width += 4;
	read_height += 4;
	clamp_width += 4;
	...
	hwCdsp_SetRawDataFormat(2);			//BGGR
	#if SENSOR_FLIP
	hwFront_SetMipiFrameSize(0xB, 0, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	//BGGR
	#else
	...
	hwCdsp_SetRawDataFormat(2); 	//RAW:0x1:RGrGbB, 0x2:BGGR;
	#if SENSOR_FLIP
	hwFront_SetMipiFrameSize(5, 1, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	
	#else
	hwFront_SetMipiFrameSize(1, 4, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);
	#endif
	sensor_SOi_h42_init(gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	
	DBG_PRINT("H42 MIPI Init Done!\r\n");
#elif (USE_SENSOR_NAME == SENSOR_OV9732)
	hwCdsp_SetRawDataFormat(2);   //BGGR
	#if SENSOR_FLIP
	hwFront_SetMipiFrameSize(0x02, 0x02, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	//pll 36,BGGR
	#else
	hwFront_SetMipiFrameSize(0x02, 0x02, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	//pll 36,BGGR
	#endif
	sensor_ov9732_init(OV9732_MIPI, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);
#elif (USE_SENSOR_NAME == SENSOR_GC1014)
	hwCdsp_SetRawDataFormat(2);			//BGGR
	#if SENSOR_FLIP
	hwFront_SetMipiFrameSize(0x9, 0, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	//BGGR
	#else
	hwFront_SetMipiFrameSize(0xC, 0, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);
	#endif
	sensor_gc1014_init(gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	
...
#if CDSP_DEBUG // Initiate TFT controller for test	
	tft_init();					 
	R_TFT_FBI_ADDR =0x80500000;//(INT32U) dummy_addr;
	R_PPU_ENABLE = 0x00200580; //for CDSP ouput YVYU
	//R_PPU_ENABLE = 0x00300580; //for CDSP output UYVY
	...
	if (CdspDev->ae_ret == AE_CHANGE) 
	{
		if(gsensor_lock_flag == 0)
		{
			...
			else
			{
				...
				if(ae_exp_set_cnt == 0)
				{
					H42_set_exposure_time(&seInfo2);
					ae_exp_set_cnt = 1;
				}
				else
				{
					H42_set_exposure_gain();
					ae_exp_set_cnt = 0;
				}
			#elif (USE_SENSOR_NAME == SENSOR_SOI_H62)
				if(ae_exp_set_cnt == 0)
				{
					H62_set_exposure_time(&seInfo2);
					H62_set_exposure_gain();
					ae_exp_set_cnt = 1;
				}
				else 
				{
					//H62_set_exposure_gain();
					H62_set_exposure_digital_gain();
					ae_exp_set_cnt = 0;
				}
				...
				if(ae_exp_set_cnt == 0)
				{
					ov9732_set_exposure_time(&seInfo2);
					ov9732_set_exposure_gain();
					ae_exp_set_cnt = 0;
				}
		 #elif (USE_SENSOR_NAME == SENSOR_GC1014)
			  gc1014_set_exposure_time(&seInfo2);
			  gc1014_set_exposure_gain();
			  ae_exp_set_cnt = 0;	
		  #endif
