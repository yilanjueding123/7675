...
void MipiIF_Setting(INT16U mipi_type, INT32U width, INT32U height)	/* 0: Raw 1:YUV */
{
	...
	intpl_height = height+4; //Enabel new denoise need 2 line
	intpl_width = width+4; //Left need shift 4 pixel column
	/*-----------program Mclk IO -------------------*/
...
/*
	...
	R_FUNPOS1 |= 0x04; // CSI Position is IOD6/IOD8/IOD9
	#endif
*/
...
#if 0//(sensor_format != OV9732_MIPI)
	(*(volatile unsigned *)0xD0F00000) |= 0x00000001; //P_GLB_CSR ,Enable MIPI
	(*(volatile unsigned *)0xD0F0000c)  = 0x00012007; //P_ECC_ORDER
#elif (sensor_format == GC1014_MIPI)
	(*(volatile unsigned *)0xD0F00000) |= 0x00000021; //P_GLB_CSR ,Enable MIPI
	(*(volatile unsigned *)0xD0F0000c)  = 0x00012007;//0x00010807; //P_ECC_ORDER gc fae abner test
#else
	...
	{
		(*(volatile unsigned *)0xD0F00014) |= (intpl_height & 0xFFFF) << 16 | ((intpl_width << 1) & 0xFFFF);//0x01e00500; //P_IMAGE_SIZE
	}
	else 
	{
		(*(volatile unsigned *)0xD0F00014) |= (intpl_height & 0xFFFF) << 16 | (intpl_width & 0xFFFF);//0x01e00280;
	}
	#if (sensor_format == GC1004_MIPI ||sensor_format == GC1014_MIPI)
		#if (SENSOR_INTERFACE == MIPI_RAW8)
		(*(volatile unsigned *)0xD0F00020) =0xB1;		//Data Format force RAW8
		#else
		(*(volatile unsigned *)0xD0F00020) =0xC1;		//Data Format force RAW10
		#endif
	#elif (sensor_format == SOI_H22_MIPI)
		#if (SENSOR_INTERFACE == MIPI_RAW8)
		(*(volatile unsigned *)0xD0F00020) =0xB0; //Data Format auto switch for H22_mipi
		#else
		(*(volatile unsigned *)0xD0F00020) =0x80; //Data Format auto switch for H22_mipi	
		#endif
	#else 
	(*(volatile unsigned *)0xD0F00020) =0x80; //Data Format auto switch
	#endif
	//front&back porch
	#if (sensor_format != OV9732_MIPI)
	(*(volatile unsigned *) 0xD0F00010) |= 0x00010404;
	#else
	(*(volatile unsigned *) 0xD0F00010) |= 0x00010800;
	#endif
...
/***************************
*For AE Hold switch at VD
****************************/
void hwCdsp_EnableVD(INT8U ae_vd_en,INT8U vdupdate_en)
{
	R_CDSP_AE_AWB_WIN_CTRL &= ~0x10;
...
void hwIsp_DenoiseWeight(INT16U rdnw1, INT16U rdnw2, INT16U rdnw3)
{
	...
	R_ISP_HRR_DENOISE_CROSSTALK_WEIGHT |= ((INT32U)rdnw1 & 0xF) |
											((INT32U)rdnw2 & 0xF) << 4 |
											((INT32U)rdnw3 & 0xF) << 8;
	//DBG_PRINT("R_ISP_HRR_DENOISE_CROSSTALK_WEIGHT = 0x%x\r\n",R_ISP_HRR_DENOISE_CROSSTALK_WEIGHT);
}
...
void hwCdsp_SetNdEdgeAmpga(INT8U ndampga)
{
	...
	//DBG_PRINT("R_Ndenoise_CTRL = 0x%x\r\n", R_Ndenoise_CTRL);
}
...
void drvl1_cdsp_init(INT32U SNR_WIDTH, INT32U SNR_HEIGHT)
{
	...
	hwCdsp_SetExtLine(ENABLE, SNR_WIDTH, 0x80);
