...
INT32S adc_record_task_create(INT8U priority)
{
	...
	err 				= OSTaskCreate(adc_record_entry, 0, (void *) &AdcRecordStack[ADC_RECORD_STACK_SIZE - 1],
		 priority);
		...
		switch (msg_id)
		{
			...
			case MSG_AUDIO_ENCODE_START:
				...
				if (pAudio_Encode_Para->bEnableDownSample)
				{
					...
					{
						pAudio_Encode_Para->DownSampleWorkMem = DownSample_Create(pAudio_Encode_Para->PCMInFrameSize,
							 pAudio_Encode_Para->DownsampleFactor);

						DownSample_Link(pAudio_Encode_Para->DownSampleWorkMem, NULL, pfn_audio_encode_once,
...
static INT32S aud_adc_double_buffer_put(INT16U * data, INT32U cwlen, OS_EVENT * os_q)
{
	...
	g_aud_adc_dma_dbf.s_addr = (INT32U)
	P_I2SRX_DATA;									//P_MIC_ASADC_DATA;
	g_aud_adc_dma_dbf.t_addr = (INT32U)
	data;
	g_aud_adc_dma_dbf.width = DMA_DATA_WIDTH_4BYTE;
	g_aud_adc_dma_dbf.count = (INT32U)
	cwlen / 2;
...
static void adc_hardware_start(INT32U SampleRate)
{
	...
	*/
	i2s_rx_init(0);
	i2s_rx_sample_rate_set(SampleRate);
	i2s_rx_mono_ch_set();
	i2s_rx_start();
...
static INT32S adc_memory_allocate(INT32U cbLen)
{
	...
	{
		g_aud_pcm_buffer[i] = (INT32U)
		gp_malloc_align(cbLen, 32);
...
static INT32S wave_encode_start(void)
{
	...
	pAudio_Encode_Para->Bit_Stream_Buffer = (INT8U *)
	gp_malloc_align(C_BS_BUFFER_SIZE, 4);
...
static int wave_encode_once(void * workmem, const short * buffer_addr, int cwlen)
{
	...
	//__msg("pAudio_Encode_Para->FileLenth: %d\n", pAudio_Encode_Para->FileLenth);
...
static INT32S wave_encode_lib_start(INT32U AudioFormat)
{
	...
	pAudio_Encode_Para->EncodeWorkMem = (INT8U *)
	gp_malloc(size);
	...
	pAudio_Encode_Para->pack_buffer = (INT8U *)
	gp_malloc_align(pAudio_Encode_Para->PackSize, 4);
...
static INT32S wave_encode_lib_stop(void)
{
	...
	nRet				= wav_enc_get_header(pAudio_Encode_Para->EncodeWorkMem, pHeader,
		 pAudio_Encode_Para->FileLenth);
	lseek(pAudio_Encode_Para->FileHandle, 0, SEEK_SET);
...
static int wave_encode_lib_once(void * workmem, const short * buffer_addr, int cwlen)
{
	...
	{
		nRet				= wav_enc_run(pAudio_Encode_Para->EncodeWorkMem, (short *) buffer_addr,
			 pAudio_Encode_Para->pack_buffer);
