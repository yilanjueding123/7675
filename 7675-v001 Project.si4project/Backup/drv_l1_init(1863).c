...
void drv_l1_init(void)
{
  ...
  #if  _DRV_L1_SYSTEM == 1
	system_clk_init();
  //打开这里虽然会降低电流，但是会影响系统稳定
  //开发板上要打开这行降低频率，小机上要屏蔽这行(头文件有做处理)
  #if 0
      if(MHZ<144)
	{
		system_set_pll(MHZ);
...
//DBG_PRINT("wwj: R_IOH_BUFFER == 0x%x\r\n", R_IOH_BUFFER);
  ...
  //#endif
	...
	tv_init();
 	  ...
 	  i2c_init();
	#else  // // ノIOB4, IOB5 穘 I2C挡狦籔 I2C  PIN 侥
	  R_I2C_MISC = 0;
	#endif
  #endif
    ...
    R_SYSTEM_CKGEN_CTRL |= 0x04;  // 開 HDMI clk  (PLL148M_EN) 
	R_SYSTEM_PLLEN |= 0x80;  // 開 HDMI clk ( HDMI_CKEN) 
